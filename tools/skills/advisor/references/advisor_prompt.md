...
**Final reconstruction** produced by Opus 4.8 advisor in a fresh session, after Opus
itself graded the prior draft at ~95% fidelity and identified four specific corrections.
This is Opus's own consolidated verbatim version with those corrections applied.

**Note on punctuation:** Opus indicates actual framing uses `--` (double hyphen).
Em-dashes (—) are preserved here for readability; substitute `--` for true verbatim.
